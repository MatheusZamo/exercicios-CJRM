...
const movingMouse =  event => {
  const setX = event.offsetX
  const setY = event.offsetY
  divEgg.innerText = `Eixo X: ${setX} | Eixo Y: ${setY}`
}

divEgg.addEventListener('mousemove',movingMouse)
/*
  06
...
const newColor = () => divEgg.style.background = "lightgoldenrodyellow"
...
const checkFront = people.some(person => person.profession === 'Front-end developer')
