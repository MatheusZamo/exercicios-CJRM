...
const greaterThan50 = randomNumbers.find(number => number > 50)
...
const sentence = ingredients.reduce((accumulator,item,index,array ) => {
  const correctWordGender = item[item.length - 1] === 'a'? 'cozida':'cozido'
  const isLastItem = index === array.length - 1
  const ingredientMessage = accumulator + `${item} ${correctWordGender}`
