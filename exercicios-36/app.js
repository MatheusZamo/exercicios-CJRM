...
const sumArrayItems = number => accumulator+= number
...
const manipulateCardClasses = (correctCardIndex) => {
  cards
    .forEach(card => card.classList.remove('carousel__item--visible'))

  cards[correctCardIndex].classList.add('carousel__item--visible')
}
  

nextButton.addEventListener('click', () => {
  const correctCardIndex = currentCardIndex === lastCardIndex
    ? currentCardIndex = 0
    : ++currentCardIndex

  manipulateCardClasses(correctCardIndex)
})
